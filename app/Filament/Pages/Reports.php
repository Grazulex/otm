...
class Reports extends Page implements Forms\Contracts\HasForms
{
    ...
    public function submit()
    {
        if ($this->ReportType == 'customervsitems') {
            $plates = Plate::select(DB::raw('created_at, origin, customer, product_type, type, is_cod, is_rush, amount, datas->>"$.payment_method" as payment_method, datas->>"$.price" as price'))->whereBetween('created_at', [$this->StartDate, $this->StopDate])->orderBy('created_at')->orderBy('origin')->orderBy('customer')->get()->toArray();
